...
{
	name = "SMAC: Anti-Wallhack Smoke",
	author = SMAC_AUTHOR,
	description = "Advanced smoke fix and anti-wallhack for CSS v34",
...
new bool:bIsClientBoughtSmokeGrenade[MAXPLAYERS+1];
...
#define DURATIONSMOKE 22.0
#define DURATIONSM 22.0
InitPrecache()
{
	...
	AddFileToDownloadsTable("models/rxg/smokevol.dx80.vtx");
	...
	AddFileToDownloadsTable("models/rxg/smokevol.sw.vtx");
...
public OnPluginStart()
{
	...
	hCvar = SMAC_CreateConVar("smac_wallhack_smoke", "1", "Enable Anti-Wallhack. This will increase your server's CPU usage.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_iCacheTicks = TIME_TO_TICK(0.25);
...
public Action:OnSmokegrenadeDetonate(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		g_iCacheTicks = TIME_TO_TICK(2.0);
		decl Float:fPos[3];
		fPos[0] = GetEventFloat(event, "x");
		fPos[1] = GetEventFloat(event, "y");
		fPos[2] = GetEventFloat(event, "z");
		new iEntity = CreateEntityByName("env_particlesmokegrenade", -1);
		if (iEntity != -1)
		{
			SetEntProp(iEntity, Prop_Data, "m_CurrentStage", 1, 4, 0);
			SetEntPropFloat(iEntity, Prop_Data, "m_FadeStartTime", 17.0, 0);
			SetEntPropFloat(iEntity, Prop_Data, "m_FadeEndTime", 22.0, 0);
			DispatchSpawn(iEntity);
			ActivateEntity(iEntity);
			TeleportEntity(iEntity, fPos, NULL_VECTOR, NULL_VECTOR);
			CreateTimer(DURATIONSMOKE, Timer_KillVolume, EntIndexToEntRef(iEntity), TIMER_FLAG_NO_MAPCHANGE);
		}
		
		iEntity = CreateEntityByName("env_particlesmokegrenade", -1);
		if (iEntity != -1)
		{
			SetEntProp(iEntity, Prop_Data, "m_CurrentStage", 1, 4, 0);
			SetEntPropFloat(iEntity, Prop_Data, "m_FadeStartTime", 17.0, 0);
			...
			ActivateEntity(iEntity);
			TeleportEntity(iEntity, fPos, NULL_VECTOR, NULL_VECTOR);
			CreateTimer(DURATIONSM, Timer_KillVolume, EntIndexToEntRef(iEntity), TIMER_FLAG_NO_MAPCHANGE);
		}
		...
		if (iEntity != -1)
		{
			...
			CreateTimer(DELAY, Timer_StartVolume, EntIndexToEntRef(iEntity), TIMER_FLAG_NO_MAPCHANGE);
			TeleportEntity(iEntity, fPos, NULL_VECTOR, NULL_VECTOR);
			CreateTimer(DURATION, Timer_KillVolume, EntIndexToEntRef(iEntity), TIMER_FLAG_NO_MAPCHANGE);
		}
	}
...
public bool:Hook_OnCollision(entity, collisiongroup, contentsmask, bool:originalResult)
{
	// Grenades and bullets should not clip.
	return !(collisiongroup == 13 || collisiongroup == 0);
}

public Action:Timer_KillVolume(Handle:timer, any:ref)
{
...
public Action:Timer_StartVolume(Handle:timer, any:ref)
{
	new ent = EntRefToEntIndex(ref);
	if (ent == INVALID_ENT_REFERENCE)
	{
	...
	if (IsValidEntity(ent))
	{
		...
		SDKHook(ent, SDKHook_ShouldCollide, Hook_OnCollision);
...
{
	new iIndex = -1;
	while ((iIndex = FindEntityByClassname(iIndex, "env_fog_controller")) != -1)
	{
		AcceptEntityInput(iIndex, "TurnOff", -1, -1, 0);
		AcceptEntityInput(iIndex, "Kill", -1, -1, 0);
	}
	iIndex = -1;
	while ((iIndex = FindEntityByClassname(iIndex, "func_smokevolume")) != -1)
	{
		AcceptEntityInput(iIndex, "Kill", -1, -1, 0);
	}
	iIndex = -1;
	while ((iIndex = FindEntityByClassname(iIndex, "func_dustmotes")) != -1)
	{
		AcceptEntityInput(iIndex, "Kill", -1, -1, 0);
	}
	return Plugin_Continue;
...
public Action:OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (iClient)
	{
		bIsClientBoughtSmokeGrenade[iClient] = false;
	}
